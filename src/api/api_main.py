...
from core.engine import ENGINE_VERSION, GLUCOSE_MODEL_VERSION, CAFFEINE_MODEL_VERSION, _get_git_commit
...
class SimulationRequest(BaseModel):
    ...
    subpopulation: Optional[str] = Field(
        None, 
        description="Filtro de población: 'athletes', 'prediabetic', 'elderly', etc."
    )
...
def results_to_response(results: PopulationResults, include_curves: bool = False) -> SimulationResponse:
    ...
    response = SimulationResponse(
        ...
        valid_percentage=results.n_valid / results.n_individuals * 100 if results.n_individuals > 0 else 0,
    ...
    **Ejemplo de uso:**
    ...
    try:
        ...
        if results_a.risk_analysis and results_b.risk_analysis:
            comparison['hyperglycemia_diff_pct'] = (
                results_a.risk_analysis.get('pct_hyperglycemia', 0) -
...
async def list_subpopulations():
    ...
    return {
        "subpopulations": [
            ...
            {"id": "prediabetic", "description": "Elevated fasting glucose, reduced insulin sensitivity"},
