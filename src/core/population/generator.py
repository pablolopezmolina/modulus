...
class PopulationGenerator:
    ...
    def _generate_continuous_lhs(
        ...
        for s in ["male", "female"]:
            mask = sex == s
            if mask.any():
                dist = self.distributions.get_sex_specific_distribution("demographics", "height_cm", s)
        ...
        for s in ["male", "female"]:
            mask = sex == s
            if mask.any():
                dist = self.distributions.get_sex_specific_distribution("demographics", "weight_kg", s)
        ...
        glucose_dist = self._get_distribution_with_override(config, "glucose_metabolism", "fasting_glucose_mg_dl")
        ...
        insulin_dist = self.distributions.get_distribution("glucose_metabolism", "fasting_insulin_mu_l")
        ...
        si_dist = self._get_distribution_with_override(config, "glucose_metabolism", "insulin_sensitivity_factor")
        ...
        for geno in ["slow", "normal", "fast"]:
            mask = cyp1a2 == geno
            if mask.any():
                dist = self.distributions.get_genotype_specific_distribution("half_life_hours", geno)
    ...
    def _generate_continuous_random(
        ...
        # Age
        age_dist = self._get_distribution_with_override(config, "demographics", "age")
        results["age"] = sample_distribution(age_dist.dist_type, age_dist.params, rng, n).astype(int)
        ...
        # Fasting glucose
        glucose_dist = self._get_distribution_with_override(config, "glucose_metabolism", "fasting_glucose_mg_dl")
        results["fasting_glucose_mg_dl"] = sample_distribution(glucose_dist.dist_type, glucose_dist.params, rng, n)
        
        # Fasting insulin
        insulin_dist = self.distributions.get_distribution("glucose_metabolism", "fasting_insulin_mu_l")
        results["fasting_insulin_mu_l"] = sample_distribution(insulin_dist.dist_type, insulin_dist.params, rng, n)
        
        # Insulin sensitivity
        si_dist = self._get_distribution_with_override(config, "glucose_metabolism", "insulin_sensitivity_factor")
        results["insulin_sensitivity_factor"] = sample_distribution(si_dist.dist_type, si_dist.params, rng, n)
        
        # Beta cell function
        results["beta_cell_function"] = self._sample_beta_cell_function(rng.random(n), config)
        
        # Gastric emptying speed
        results["gastric_emptying_speed"] = self._sample_gastric_emptying_speed(rng.random(n), config)
        ...
        # Habitual caffeine
        caffeine_dist = self._get_habitual_caffeine_distribution(config)
        results["habitual_caffeine_mg"] = sample_distribution(caffeine_dist.dist_type, caffeine_dist.params, rng, n)
    ...
    def _get_habitual_caffeine_distribution(self, config: GenerationConfig) -> DistributionSpec:
        ...
        return self.distributions.get_distribution("caffeine_metabolism", "habitual_caffeine_mg_day")
    ...
    def _apply_correlations(
        ...
        if abs(config.height_weight_corr) > 0.01:
            params = self._apply_rank_correlation(
                params, "height_cm", "weight_kg", 
                config.height_weight_corr, rng
            )
        
        # === 2. Correlación Glucose-Insulin ===
        if abs(config.glucose_insulin_corr) > 0.01:
            params = self._apply_rank_correlation(
                params, "fasting_glucose_mg_dl", "fasting_insulin_mu_l",
                config.glucose_insulin_corr, rng
            )
