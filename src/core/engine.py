...
class StreamingAggregator:
    ...
    def _sample_curve(self, result: 'IndividualResult') -> None:
        ...
        if len(self._curve_samples) < max_samples:
            self._curve_samples.append({
                ...
                'alertness': np.array(result.alertness_curve) if result.alertness_curve else None,
            })
        else:
    ...
    def get_risk_analysis(self) -> Dict[str, float]:
        ...
        return {
            f'pct_{k}': v / self.n_valid * 100 
            for k, v in self._risk_counts.items()
        }
            ...
            for key, g in groups.items():
                ...
                if n > 0:
                    result[key] = {
                        ...
                        'avg_peak_glucose': g['glucose_peak'].mean if g['glucose_peak'].n > 0 else 0,
                        'avg_alertness': g['alertness_peak'].mean if g['alertness_peak'].n > 0 else 0,
                    }
            return result
    ...
    def get_curve_percentiles(self, time_points: List[float]) -> Tuple[Dict, Dict, Dict]:
        ...
        glucose_curves = [
            s['glucose'] for s in self._curve_samples 
        ...
        # Cafeína
        caffeine_curves = [
            s['caffeine'] for s in self._curve_samples
        ...
        # Alertness
        alertness_curves = [
            s['alertness'] for s in self._curve_samples
...
class SimulationEngine:
    ...
    def run(
        ...
        if population is None:
            logger.info(f"🔬 Generating population: n={config.population_size}, subpop={config.subpopulation}")
        ...
        valid_pct = results.n_valid / results.n_individuals * 100 if results.n_individuals > 0 else 0
        logger.info(f"✅ Complete in {elapsed:.1f}s ({results.n_valid}/{results.n_individuals} valid, {valid_pct:.1f}%)")
    ...
    def _simulate_individual(
        ...
        if config.store_timeseries and time_points is not None:
            result.time_points = time_points.tolist() if isinstance(time_points, np.ndarray) else time_points
            if glucose_curve is not None:
                result.glucose_curve = glucose_curve.tolist() if isinstance(glucose_curve, np.ndarray) else glucose_curve
            if caffeine_curve is not None:
                result.caffeine_curve = caffeine_curve.tolist() if isinstance(caffeine_curve, np.ndarray) else caffeine_curve
            if alertness_curve is not None:
                result.alertness_curve = alertness_curve.tolist() if isinstance(alertness_curve, np.ndarray) else alertness_curve
    ...
    def _build_results_from_aggregator(
        ...
        time_points = list(np.arange(
            0, config.duration_minutes + config.time_step_minutes, 
    ...
    def _aggregate_results(
        ...
        if has_glucose and time_points:
            glucose_matrix = np.array([
                r.glucose_curve for r in valid_results 
        ...
        # Estadísticas escalares
        glucose_stats = {
            'peak': self._calc_scalar_stats([r.peak_glucose for r in valid_results if r.peak_glucose > 0]),
            'auc': self._calc_scalar_stats([r.auc_glucose for r in valid_results if r.auc_glucose > 0]),
            'time_to_peak': self._calc_scalar_stats([r.time_to_peak_glucose for r in valid_results if r.time_to_peak_glucose > 0]),
        }
        
        caffeine_stats = {
            'peak': self._calc_scalar_stats([r.peak_caffeine for r in valid_results if r.peak_caffeine > 0]),
            'auc': self._calc_scalar_stats([r.auc_caffeine for r in valid_results if r.auc_caffeine > 0]),
            'alertness_peak': self._calc_scalar_stats([r.alertness_peak for r in valid_results if r.alertness_peak > 0]),
        }
        
        # Risk analysis
        risk_analysis = {
            'pct_hyperglycemia': sum(1 for r in valid_results if r.hyperglycemia_flag) / n_valid * 100,
            'pct_severe_hyperglycemia': sum(1 for r in valid_results if r.severe_hyperglycemia_flag) / n_valid * 100,
            'pct_hypoglycemia': sum(1 for r in valid_results if r.hypoglycemia_flag) / n_valid * 100,
            ...
            'pct_sleep_disruption': sum(1 for r in valid_results if r.sleep_risk_flag) / n_valid * 100,
        }
    ...
    def _analyze_by_bmi(
        ...
        for cat, group_results in groups.items():
            ...
            if n > 0:
                analysis[cat] = {
                    'n': n,
                    'pct_hyperglycemia': sum(1 for r in group_results if r.hyperglycemia_flag) / n * 100,
                    'pct_crash_risk': sum(1 for r in group_results if r.crash_risk_flag) / n * 100,
                    'avg_peak_glucose': float(np.mean([r.peak_glucose for r in group_results if r.peak_glucose > 0])),
                }
        return analysis
        ...
        for act, group_results in groups.items():
            ...
            if n > 0:
                analysis[act] = {
                    'n': n,
                    'pct_hyperglycemia': sum(1 for r in group_results if r.hyperglycemia_flag) / n * 100,
                    'avg_alertness': float(np.mean([r.alertness_peak for r in group_results if r.alertness_peak > 0])),
                }
        return analysis
