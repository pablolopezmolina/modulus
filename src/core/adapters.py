    ...
    def get_effect_summary(
        ...
        if product.fat_g > 0:
            summary['combined_factors']['fat_kmax_factor'] = max(
                c.fat_kmax_min_factor,
                1.0 - (product.fat_g / 10.0) * c.fat_kmax_reduction_per_10g
            )
...
class CaffeineModelAdapter:
    ...
    def create_input(
        ...
        tolerance_days = int(person.habitual_caffeine_mg / 100) if person.habitual_caffeine_mg > 0 else 0
